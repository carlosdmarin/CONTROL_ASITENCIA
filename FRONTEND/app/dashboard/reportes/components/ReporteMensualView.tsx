...
export function ReporteMensualView({ reporte }: { reporte: ReporteMensualResponse }) {
  ...
  const showNA = r.horasProgramadas == null || Number(r.horasProgramadas) === 0;
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between p-6 border-b border-slate-200 bg-white">
        ...
        <div className="text-right">
          <p className="text-sm font-bold tracking-tight text-slate-900">REPORTE MENSUAL DE ASISTENCIA</p>
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge variant="outline" className="bg-slate-50 capitalize">{reporte.mesLabel}</Badge>
            <Badge variant="outline" className="bg-slate-50">Tipo: Mensual</Badge>
            <Badge variant="outline" className="bg-slate-50">{reporte.detalleDiario.length} días</Badge>
          </div>
          <p className="text-xs text-slate-500 mt-1">{formatFechaLarga(reporte.mesInicio)} → {formatFechaLarga(reporte.mesFin)}</p>
        </div>

        {/* Resumen mensual */}
        <div>
          <h3 className="text-[11px] font-bold tracking-widest text-slate-500">RESUMEN MENSUAL</h3>
          <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <Card className="bg-slate-50 border-slate-200">
              <CardContent className="p-3 text-center">
                ...
                <p className="text-lg font-bold text-slate-900">{showNA ? "N/A" : `${Number(r.porcentajeCumplimiento).toFixed(1)}%`}</p>
              </CardContent>
            </Card>
          </div>
        </div>
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-4 gap-3">
            <Card className="bg-slate-50 border-slate-200">
              <CardContent className="p-3 text-center">
                ...
                <p className="text-sm font-bold text-slate-900">{showNA ? "—" : formatHoras(r.horasProgramadas)}</p>
              </CardContent>
            </Card>
            <Card className="bg-slate-50 border-slate-200">
              <CardContent className="p-3 text-center">
            ...
            <Card className="bg-slate-900 border-slate-900">
              <CardContent className="p-3 text-center">
                ...
                <p className="text-sm font-bold text-white">{showNA ? "N/A" : `${Number(r.porcentajeCumplimiento).toFixed(1)}%`}</p>
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        {/* Detalle */}
        <div>
          <h3 className="text-[11px] font-bold tracking-widest text-slate-500">DETALLE DIARIO — {reporte.mesLabel.toUpperCase()} ({reporte.detalleDiario.length} días)</h3>
            ...
            <div className="min-w-[900px]">
              ...
              {reporte.detalleDiario.map((d) => (
                <div key={d.fecha} className="grid grid-cols-9 text-center text-xs divide-x divide-slate-200 border-t border-slate-200">
                  <div className="p-2 font-mono text-slate-600 text-[11px]">{formatFechaCorta(d.fecha)}</div>
                  <div className="p-2 font-medium text-slate-900 text-[11px]">{d.diaSemana}</div>
        ...
        {/* Incidencias */}
        <div>
          <h3 className="text-[11px] font-bold tracking-widest text-slate-500">INCIDENCIAS MENSUALES</h3>
          {reporte.incidencias && reporte.incidencias.length > 0 ? (
            ...
            <p className="mt-2 text-xs text-slate-400 italic">Sin incidencias registradas.</p>
          )}
        </div>
